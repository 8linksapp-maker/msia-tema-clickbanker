...
export default function PromoBarEditor() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center p-16 text-ink-faint">
    ...
    <div className="max-w-3xl space-y-6 pb-32">
      <div className="flex items-center justify-between bg-white p-5 px-7 rounded-2xl border border-border shadow-sm">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${promo.enabled ? 'bg-primary-soft text-primary' : 'bg-elev text-ink-faint'}`}>
            <Megaphone className="w-5 h-5" />
          </div>
          <div>
            <h2 className="text-lg font-bold text-ink">Promo Bar</h2>
            <p className="text-xs text-ink-muted mt-0.5">Faixa promocional no topo do site</p>
          </div>
        </div>
        <button onClick={save} disabled={saving} className="bg-ink hover:bg-primary disabled:bg-rule text-white px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2">
      ...
      {/* Live preview */}
      <div>
        <p className="text-[10px] font-bold text-ink-muted uppercase tracking-wider mb-2">Preview</p>
        <div className={`relative rounded-xl overflow-hidden ${promo.enabled ? '' : 'opacity-50'}`}>
          <div className={`${promo.background === 'ink' ? 'bg-ink text-white' : 'bg-primary text-white'} px-6 py-3`}>
          ...
          {!promo.enabled && (
            <div className="absolute inset-0 flex items-center justify-center bg-elev/80">
              <span className="bg-white px-3 py-1 rounded mono text-xs font-bold text-ink-muted uppercase">desligada</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-border shadow-sm">
        <label className="flex items-center justify-between cursor-pointer">
          <div>
            <p className="font-bold text-ink text-base">Promo bar ativa</p>
            <p className="text-xs text-ink-muted mt-1">Exibe a faixa no topo de todas as páginas</p>
          </div>
          <div className="relative">
            <input type="checkbox" checked={promo.enabled} onChange={e => setPromo({ ...promo, enabled: e.target.checked })} className="sr-only peer" />
            <div className="w-14 h-7 bg-border rounded-full peer-checked:bg-primary transition-colors"></div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-border shadow-sm space-y-4">
        <h3 className="font-bold text-ink mb-2">Conteúdo</h3>
      ...
      {/* Style */}
      <div className="bg-white p-6 rounded-2xl border border-border shadow-sm space-y-4">
        <h3 className="font-bold text-ink mb-2">Estilo</h3>

        <Field label="Cor de fundo">
          <div className="flex gap-2">
            <button onClick={() => setPromo({ ...promo, background: 'primary' })} className={`flex-1 px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${promo.background === 'primary' ? 'bg-primary text-white ring-2 ring-primary ring-offset-2' : 'bg-elev text-ink-muted'}`}>
              Primary (terracota)
            </button>
            <button onClick={() => setPromo({ ...promo, background: 'ink' })} className={`flex-1 px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${promo.background === 'ink' ? 'bg-ink text-white ring-2 ring-ink-muted ring-offset-2' : 'bg-elev text-ink-muted'}`}>
              Ink (preto)
            </button>
          </div>
        </Field>
        ...
        <label className="flex items-center gap-2 cursor-pointer text-sm pt-2">
          <input type="checkbox" checked={promo.dismissible} onChange={e => setPromo({ ...promo, dismissible: e.target.checked })} className="w-4 h-4 accent-primary" />
          <span className="text-ink-muted">Visitante pode fechar (X no canto)</span>
        </label>
      </div>
