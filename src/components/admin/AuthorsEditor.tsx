...
export default function AuthorsEditor() {
    ...
    const saveModalAuthor = async () => {
        if (!tempAuthor?.name?.trim()) { triggerToast('O nome do autor é obrigatório!', 'error'); return; }
        const arr = [...authors];
    ...
    const removeAuthor = async (index: number) => {
        if (!(await confirmDialog({ title: 'Excluir este autor?', confirmLabel: 'Excluir', danger: true }))) return;
        const arr = [...authors];
        arr.splice(index, 1);
        setAuthors(arr);
        await saveToGithub(arr);
